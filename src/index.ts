...
function nameShortner(fullName, maxNameLength) {
  ...
  if (nameSlicedSplitted[index] === nameSplitted[index]) {
    lastName = nameSplitted[index];
  }
  nameSlicedSplitted[index] = lastName;
  let nameSlicedSplittedFiltered = nameSlicedSplitted.filter(
    (item) => item !== "undefined."
  );
...
let maxNameWidth =
  container.offsetWidth - (logoWidth + logOutWidth + TOLERANCE);
//<--- Definindo tamanho máximo do nome
